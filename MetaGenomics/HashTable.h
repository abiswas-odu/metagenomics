...
class HashTable{
	private:
		...
		bool insertIntoTable(const Read *read, const string & substring, const UINT64 & orientation, omp_lock_t *lock);	// Insert a string in the hash table.
		bool hashRead(const Read *read, omp_lock_t *lock); 					// Ted: Hash prefix and suffix of the read and its reverse complement in the hash table. Turn over to the constant
